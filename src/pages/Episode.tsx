...
import { MessageSquare, Send, Loader2, List, Reply, X, ThumbsUp, ThumbsDown, Share2 } from 'lucide-react';
...
export default function Episode() {
  ...
  const [commentName, setCommentName] = useState(localStorage.getItem('ph_anon_name') || '');
  ...
  // FIX: Definitions for root comments and reply logic
  ...
  const { data: episode, isLoading: loading } = useQuery({
  ...
  if (!episode) return <div className="min-h-screen bg-black flex items-center justify-center text-white">Error loading episode.</div>;
  ...
  return (
    <div className="min-h-screen pb-24 bg-background text-foreground font-sans">
      <Helmet>
        <title>{`Nonton ${episode.donghua_details?.title || ''} ${episode.episode} Sub Indo HD`}</title>
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "description": `Nonton ${episode.donghua_details?.title} ${episode.episode} Subtitle Indonesia terbaru kualitas HD gratis.`,
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="flex flex-wrap gap-4 mb-10 items-center justify-between">
          <div className="flex flex-wrap gap-4 items-center">
            {/* SERVER SELECTOR - BALIK LAGI BRO! */}
            ...
            <div className="flex gap-2">
                <Button size="sm" variant="outline" className="rounded-full border-white/20 px-6" onClick={() => prevEpisode && navigate(`/episode/${prevEpisode.slug}`)} disabled={!prevEpisode}>Prev</Button>
                <Button size="sm" variant="outline" className="rounded-full border-white/20 px-6" onClick={() => nextEpisode && navigate(`/episode/${nextEpisode.slug}`)} disabled={!nextEpisode}>Next</Button>
            </div>
          </div>
